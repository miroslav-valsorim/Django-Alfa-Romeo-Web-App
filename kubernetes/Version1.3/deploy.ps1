...
Write-Host "  $count. Running database migrations (Job)..."
kubectl apply -f migrate-job.yml
Write-Host "  Waiting for migrations to complete..."
try {
    kubectl wait --for=condition=complete job/django-migrate -n alfa-romeo --timeout=120s 2>&1 | Out-Null
    Print-Success "Migrations complete"
} catch {
    Print-Warning "Migration job timed out. Check logs: kubectl logs job/django-migrate -n alfa-romeo"
}
$count++

Write-Host "  $count. Applying Redis Deployment..."
kubectl apply -f redis-deployment.yml
$count++

Write-Host "  $count. Applying Redis Service..."
...
Write-Host ""
Write-Host "  Check migration job logs:"
Write-Host "    kubectl logs job/django-migrate -n alfa-romeo"
Write-Host ""
...
Write-Host ""
Write-Host "  Ping Redis:"
Write-Host "    kubectl exec -it deployment/redis -n alfa-romeo -- redis-cli ping"
Write-Host ""
